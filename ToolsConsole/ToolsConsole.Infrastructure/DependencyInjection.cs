...
public static class DependencyInjection
{
    ...
    {
        var connectionString = ConnectionStringDecoder.DecodeTripleBase64IfNeeded(
        ...
        services.AddSingleton<ISystemConfigProvider, DbSystemConfigProvider>();
        services.AddSingleton<IPasswordHasher, PasswordHasher>();

        // 存储密钥：加解密落库的密文（默认注入，Application 层使用）
        services.AddSingleton<IConnectionStringCryptor>(
            provider => new AesGcmCryptor(
                provider.GetRequiredService<ISystemConfigProvider>(),
                "Security:EncryptionKey"));

        // 分发密钥：取串接口对返回链接串加密（与调用方共享，生产环境务必单独配置）
        services.AddKeyedSingleton<IConnectionStringCryptor>(
            CryptorKeys.Delivery,
            (provider, _) => new AesGcmCryptor(
                provider.GetRequiredService<ISystemConfigProvider>(),
                "Security:DeliveryKey"));

        // 传输解密：登录/重置密码等场景解密前端提交的密码密文
        services.AddSingleton<ITransportPasswordDecryptor>(
            provider => new AesCbcPasswordDecryptor(provider.GetRequiredService<ISystemConfigProvider>()));
